...
const isSameDayInTimezone = (date1, date2, tz) => {
  const d1 = new Date(date1);
  const d2 = new Date(date2);
  const fmt = "yyyy-MM-dd";
  return format(d1, fmt, { timeZone: tz }) === format(d2, fmt, { timeZone: tz });
};
...
exports.CheckIn = (req, res) => {
  ...
  const existingRecord = presensiRecords.find(
    (record) =>
      ...
      isSameDayInTimezone(record.checkIn, waktuSekarang, timeZone)
  );
